...
fn srgb_to_rgbvals(srgb: Srgb<f32>) -> RgbWrapper {
    RgbWrapper {
        red: srgb.red,
        green: srgb.green,
        blue: srgb.blue,
...
impl From<RgbWrapper> for Vec4 {
    fn from(value: RgbWrapper) -> Self {
        vec4(value.red, value.green, value.blue, 1.0)
    }
}

pub trait TrypColor {
    fn lerp(&self, other: &Self, val: f32) -> Self;
    fn to_rgb(&self) -> RgbWrapper;
    fn to_gpu_color(&self) -> Vec4 {
        Vec4::from(self.to_rgb())
    }
}

impl TrypColor for RgbWrapper {
    fn to_rgb(&self) -> RgbWrapper {
        *self
    }
    fn lerp(&self, other: &Self, val: f32) -> Self {
        // Perform linear interpolation on each color channel with rounding
        let red = self.red + val * (other.red - self.red);
        let green = self.green + val * (other.green - self.green);
        let blue = self.blue + val * (other.blue - self.blue);

        RgbWrapper { red, green, blue }
    }
}
...
pub type RgbColorScheme = Vec<RgbWrapper>;
...
pub fn generate_rainbow_gradient_constant_chroma(size: usize) -> Vec<RgbWrapper> {
    generate_generic_gradient(size, |i| {
        Oklch::new(0.7, 0.16, ((i + 4) * (360 / size) % 360) as f32)
    })
}

pub fn generate_rainbow_gradient(size: usize) -> Vec<RgbWrapper> {
    generate_generic_gradient(size, |i| {
...
pub fn generate_lerp_gradient(size: usize, color_points: &[impl TrypColor]) -> Vec<RgbWrapper> {
    let num_colors = color_points.len();
    if num_colors == 0 {
        return vec![RgbWrapper::default(); size];
    }
...
pub fn generate_circular_ocean_blues(size: usize) -> Vec<RgbWrapper> {
...
pub fn generate_circular_sunset_orange(size: usize) -> Vec<RgbWrapper> {
    let colors = [
...
pub fn generate_circular_forest_greens(size: usize) -> Vec<RgbWrapper> {
    let colors = [
...
pub fn generate_circular_purple_dream(size: usize) -> Vec<RgbWrapper> {
    let colors = [
